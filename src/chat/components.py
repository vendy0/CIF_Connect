...
class BaseChatMessage(ft.Row):
    def __init__(self, message: Message, page: ft.Page, on_copy, on_reply, on_edit, on_report, on_react, on_delete):
        ...
        self.timestamp_text = self.message.message_time.strftime("%H:%M")
        # Si le message n'est pas d'aujourd'hui, on ajoute la date
        if self.message.message_date != datetime.now().date():
            self.timestamp_text = f"{self.message.message_date.strftime('%d/%m')}  \t\t  {self.timestamp_text}"
        ...
        self.time_row = ft.Row(
            [self.modified_text, ft.Text(self.timestamp_text, size=10), self.status_icon],
            alignment="end",
            spacing=2,
        )
    ...
    def update_status(self):
        """Remplace le loader par un check une fois le message validé"""
        self.status_icon.content = ft.Icon(ft.Icons.CHECK, size=12, color=ft.Colors.OUTLINE)
        self.update()
            ...
            content=ft.Container(
                content=ft.Column(
                    [
                        ...
                        self.content_text,
                        ft.Row(
                            [self.modified_text, ft.Text(self.timestamp_text, size=10)],
                            alignment="end",
                            spacing=2,
                        ),
                    ],
