...
#             # Timeout court pour échouer vite, pas en 15s
#             timeout=httpx.Timeout(connect=3.0, read=8.0, write=5.0, pool=3.0),
#             # Désactive le keep-alive qui crée les connexions mortes
#             limits=httpx.Limits(
#                 max_keepalive_connections=0,  # <-- La clé du problème
#                 max_connections=10,
#             ),
#         )
...
def format_date(date_to_format):
    today = date.today()
    if isinstance(date_to_format, datetime):
        d = date_to_format.date()
    else:
        d = date_to_format
