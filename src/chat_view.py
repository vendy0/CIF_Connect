...
class BaseChatMessage(ft.Row):
    def __init__(self, message: Message, page: ft.Page, on_reply, on_edit, on_report, on_react, on_delete=None):
        super().__init__()
        self.message = message
        self._page_ref = page
        self.on_reply = on_reply
        ...
        self.controls_builder = []
        self.controls_builder.extend(
            [
                ft.Stack(
                    [
                        ft.Column([bubble, ft.Container(height=10 if self.message.reactions else 0)], tight=True),
            ...
            if self.message.parent_author and self.message.parent_content
            else ft.Container()
        )
...
class MyChatMessage(BaseChatMessage):
    def __init__(self, **kwargs):
        ...
        self.menu_items = [
            ft.ListTile(leading=ft.Icon(ft.Icons.EDIT), title=ft.Text("Modifier"), on_click=lambda e: self._page_ref.run_task(self.on_edit, e, self.message))
            if (datetime.now() - timedelta(minutes=15)) < self.message.message_datetime
            else ft.Container(),
            ft.ListTile(leading=ft.Icon(ft.Icons.DELETE_OUTLINE, color="error"), title=ft.Text("Supprimer"), on_click=lambda e: self._page_ref.run_task(on_delete, e, self.message)),
        ...
        bubble = ft.GestureDetector(
            ...
            content=ft.Container(
                content=ft.Column(
                    [
                        # Pas besoin du pseudo ici, c'est nous
                        ft.Text(self.message.pseudo, size=12, weight="bold", color=get_avatar_color(self.message.pseudo, COLORS_LOOKUP)),
                        ft.Text(self.message.content, size=15, selectable=True, color=ft.Colors.ON_PRIMARY_CONTAINER),
                        ft.Row(
...
class OtherChatMessage(BaseChatMessage):
    def __init__(self, **kwargs):
        ...
        self.menu_items = [
            ft.ListTile(leading=ft.Icon(ft.Icons.REPLY), title=ft.Text("Répondre"), on_click=lambda e: self._page_ref.run_task(self.action_reply, e)),
            ft.ListTile(leading=ft.Icon(ft.Icons.FAVORITE_BORDER), title=ft.Text("Réagir"), on_click=self.action_react),
            ft.ListTile(leading=ft.Icon(ft.Icons.REPORT, color="error"), title=ft.Text("Signaler"), on_click=lambda _: self.on_report(self.message)),
        ]
        ...
        self.controls_builder.extend(
            [
                ft.CircleAvatar(content=ft.Text(get_initials(self.message.pseudo)), bgcolor=get_avatar_color(self.message.pseudo, COLORS_LOOKUP)),
                ft.Stack([ft.Column([bubble, ft.Container(height=10 if self.message.reactions else 0)], tight=True), ft.Container(content=self.get_reactions_row(), bottom=0, right=10)]),
            ]
        )
            ...
            content=ft.Container(
                content=ft.Column(
                    [
                        ft.Text(self.message.pseudo, size=12, weight="bold", color=get_avatar_color(self.message.pseudo, COLORS_LOOKUP)),
                        ft.Text(self.message.content, size=15, selectable=True),
                        ft.Row([ft.Text(self.message.message_time.strftime("%H:%M"), size=10)], alignment="end"),
                    ],
                    tight=True,
                    horizontal_alignment="stretch",
                ),
