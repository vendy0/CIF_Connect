import flet as ft
from utils import generer_pseudo, get_avatar_color, host, port
import httpx
...
async def SettingsView(page: ft.Page):
    ...
    def changer_pseudo(e):
        ...
        nouveau_pseudo_input = ft.TextField(value=pseudo, read_only=True, label="Pseudo suggéré", border_color=get_avatar_color(pseudo), label_style=ft.TextStyle(color=get_avatar_color(pseudo)))
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code != 200:
                        nouveau_pseudo_input.error = response.json().get("detail", "Erreur inconnue")
                        page.update()
                        return
        ...
        def generer_un_autre(e):
            ...
            page.update()
            nouveau_pseudo_input.border_color = get_avatar_color(nouveau_pseudo_input.value)
            nouveau_pseudo_input.label_style = ft.TextStyle(color=get_avatar_color(nouveau_pseudo_input.value))
            page.update()
