...
def update_user_pseudo(db: Session, user_id: int, new_pseudo: str):
	...
	try:
		...
		db.rollback()
		raise HTTPException(
			status_code=409, detail="Ce pseudo est déjà utilisé ! Choisissez un autre."
		)
	...
	if resolution_data.ban_user and report.reported_id:
		...
		if user_to_ban:
			...
			if resolution_data.ban_duration_hours:
				user_to_ban.ban_expires_at = datetime.utcnow() + timedelta(
					hours=resolution_data.ban_duration_hours
				)
			else:
	...
	try:
		stmt = (
			...
			.options(
				...
				selectinload(Message.reactions).joinedload(
					Reaction.user
				),  # Charge les réactions ET qui a réagi
				joinedload(Message.parent).joinedload(
					Message.author
				),  # Charge le message parent et son auteur (pour l'affichage "réponse à X")
			)
...
def create_message(
	db: Session, room_id: int, message_data: MessageCreate, author_id, message_type="chat"
):
